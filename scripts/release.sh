...
do_tarball() {
  ...
  src="${ROOT}/build/meson-dist/kablamo-${VERSION}.tar.xz"
  if [ -f "$src" ]; then
    cp -f "$src" "$DISTDIR/"
    echo "tarball: ${DISTDIR}/kablamo-${VERSION}.tar.xz"
  else
    echo "meson dist did not produce kablamo-${VERSION}.tar.xz" >&2
...
do_deb() {
  ...
  mkdir -p "$DISTDIR"
  for f in "${ROOT}/../kablamo_${VERSION}"-*.deb \
           "${ROOT}/../kablamo-dbgsym_${VERSION}"-*.deb; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
    echo "deb: $DISTDIR/$(basename "$f")"
  done
  for f in "${ROOT}/../kablamo_${VERSION}"-*.buildinfo \
           "${ROOT}/../kablamo_${VERSION}"-*.changes; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
  done
}
...
do_appimage() {
  ...
  linuxdeploy --appdir "$APPDIR" \
    --executable "${APPDIR}/usr/bin/kablamo" \
    --desktop-file "${APPDIR}/usr/share/applications/kablamo.desktop" \
    --icon-file "${APPDIR}/usr/share/icons/hicolor/scalable/apps/kablamo.svg" \
    $PLUGIN_ARGS \
    --output appimage
  mkdir -p "$DISTDIR"
  for f in "${ROOT}/Kablamo-${VERSION}"-*.AppImage \
           "${ROOT}/Kablamo!-${VERSION}"-*.AppImage \
           "${ROOT}/kablamo-${VERSION}"-*.AppImage \
           "${ROOT}/build/Kablamo-${VERSION}"-*.AppImage \
           "${ROOT}"/*.AppImage; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
  done
